...
const BusinessLocationsComponents = props => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const responseLocation = await getBusinessLocationList(
        props.data.businessLocation.businessId
      );
      setState({
    ...
    fetchData();
  }, [reRender, props.data.businessLocation.businessId]);
  ...
  const handleSubmitButton = async e => {
    ...
    if (isUpdate) {
      try {
        ...
        notify({
          success: true,
          message: 'Success updating Business Location.'
        });
      } catch (error) {}
    }
    if (!isUpdate) {
  ...
  function displayBusinessLocation() {
    if (state.dataLocation) {
      return (
        <BusinessLocationCards
          data={state.dataLocation}
          cardStyle={classes}
          onDeleteBusinessLoc={showDeleteModal}
          onEditBusinessLoc={showEditModal}
        />
      );
    }
  }
