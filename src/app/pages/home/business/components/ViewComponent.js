...
const ViewComponent = () => {
  ...
  const [
    ...
    setProcessupdateBusinessProfile
  ] = useState(false);
  ...
  const HandleUpdateBusinessProfile = formDataBusinessProfile => {
    updateBusinessProfile(formDataBusinessProfile)
      .then(result => {
        ...
        notify({
          success: true,
          message: 'Success Updating Business Profile.'
        });
    ...
    if (props !== 0) {
      return (
        <BusinessProfile
          ...
          onProcessing={processupdateBusinessProfile}
        />
      );
    }
  };
  ...
  const LocationsContainer = props => {
    return <BusinessLocations data={props.data} />;
  };
